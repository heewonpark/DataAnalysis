...
def exp_single2(x, alpha, gamma):
    c = 1.5
    beta = 250
    return alpha * sp.exp(-(x-beta)/gamma)+c

def exp_double2(x, alpha1, gamma1, alpha2, gamma2):
    c = 1.5
    beta = 250
    return alpha1 * sp.exp(-(x-beta)/gamma1)  +  alpha2 * sp.exp(-(x-beta)/gamma2) + c
...
def fitting_PSTH_2000ng():
    ...
    parameter = [40,250,-150,0]
    #param_fin = optimize.fmin(cost_esr, parameter)
    param_fin = optimize.fmin(cost_es, parameter,maxiter=2000,maxfun=2000)
...
def fitting_PSTH(dose, parameters, bp_func,bp_func_cost, ap_func, ap_func_cost):
    ...
    print init_bp_fmin, param_fin
    print bp_func_cost(init_bp_fmin),bp_func_cost(param_fin)
    #x_fbp = np.arange(0,fit.psth_t[MAX_INDEX],0.01)
    x_bp = np.arange(0,300.0,0.01)
    #y_bp_cf = exp_single_rise(x_bp, *popt)
...
init_bp_curve_fit = [52,250,-70,0]
init_bp_fmin = [52,250,-70,0]
#init_bp_fmin      = [40,250,-150,0]
init_ap_curve_fit = [70,190,170,0]
init_ap_fmin      = [70,190,170,0]
param_5000ng = [init_bp_curve_fit, init_bp_fmin, init_ap_curve_fit, init_ap_fmin]
#fitting_PSTH(5000, param_5000ng, exp_single, cost_es, exp_single, cost_es)
...
init_bp_curve_fit = [15,-30]
init_bp_fmin = [15,-30]
#init_bp_fmin      = [40,250,-150,0]
init_ap_curve_fit = [70,190,170,0]
init_ap_fmin      = [70,190,170,0]
param_2000ng = [init_bp_curve_fit, init_bp_fmin, init_ap_curve_fit, init_ap_fmin]
fitting_PSTH(2000, param_2000ng, exp_single2, cost_es2, exp_single, cost_es)
