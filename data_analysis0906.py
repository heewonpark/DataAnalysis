...
w_step_time = []
w_step_freq = [0 for _ in range(160)]
DATABIN     = 0.1
AVER_STIM   = 6.0
...
def analysis_by_bin(wname,splist2,Bin, steps, delay, dt, timeWidth) :
    ...
    l_step_freq = [0 for _ in range(160)]
    ...
    plt.savefig(wname+"bin.png")
    bin_file = open(wname+"bin.dat",'w')
...
wholedata = open('ifreq_whole.dat','w')
...
w_step_time =[0 for _ in range(160)]
for i in range(160):
...
aver_f = open('average.dat','w')

for i in range(160):
    aver_f.writelines(repr(w_step_time[i])+'\t'+repr(w_step_freq[i])+'\n')

flg3 = plt.figure()
plt.bar(w_step_time, w_step_freq,width = 0.1)
plt.ylim(0,150)
plt.xlim(0,15)
plt.xlabel('Time[s]')
plt.ylabel('Frequency[Hz]')
